        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (arg.startsWith("data")) {
                ...
                try {
                    ...
                    ErrorReporter.warning("Flag " + getType() + " has 'needed' argument with invalid number: " + value);
                    continue;
                }
    ...
    protected void onRegistered() {
        ...
        while (it.hasNext()) {
            ...
            if (c.ingredient != null && Tools.findItemInIngredients(recipe, c.ingredient.getType(), c.ingredient.getDurability()) == 0) {
                ErrorReporter.error("Flag " + getType() + " couldn't find ingredient: " + ToolsItem.print(c.ingredient));
                it.remove();
            }
        }
    }
    ...
    public boolean checkIngredientConditions(ItemStack item, Args a) {
        ...
        for (Conditions cond : condList) {
            ...
            if (success) {
                ...
                if (cond.hasNeeded() && cond.neededLeft > 0) {
                    cond.neededLeft -= 1;
                }
            }
        }
        ...
        if (a.inventory() instanceof CraftingInventory) {
            ...
            while (iter.hasNext()) {
                ...
                if (checkConditions.hasNeeded()) {
                    checkConditions.neededLeft = checkConditions.needed;
                }
            }
            ...
            while (iter.hasNext()) {
                ...
                if (checkConditions.hasNeeded()) {
                    if (!a.hasReasons() && checkConditions.neededLeft > 0) {
                        a.addCustomReason("Needed items mismatch!");
                    }
                    
                    checkConditions.neededLeft = -1;
                }
            }
