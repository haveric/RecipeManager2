...
public class FlagSound extends Flag {
    ...
    static {
        ...
        A = new String[] { "{flag} <sound> | [arguments]", };

        D = new String[] { "Plays a sound at crafting location.", "Using this flag more than once will overwrite the previous flag.", "", "The <sound> argument must be a sound name, you can find them in '" + Files.FILE_INFO_NAMES + "' file at 'SOUND LIST' section.", "", "Optionally you can specify some arguments separated by | character:", "  volume <0.0 to 100.0> = (default 1.0) sound volume, if exeeeds 1.0 it extends range, each 1.0 extends range by about 10 blocks.", "  pitch <0.0 to 4.0>    = (default 0.0) sound pitch value.", "  player                = (default not set) if set it will only play the sound to the crafter.", "You can specify these arguments in any order and they're completly optional.", };

        E = new String[] { "{flag} level_up", "{flag} wolf_howl | volume 5 // can be heard loudly at 50 blocks away", "{flag} portal_travel | player | volume 0.65 | pitch 3.33", };
    }
