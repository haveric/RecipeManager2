	...
	public static boolean matches(BaseRecipe baseRecipe, Recipe bukkitRecipe) {
		...
		if (bukkitRecipe instanceof CraftFurnaceRecipe) {
			...
			if (baseRecipe instanceof SmeltRecipe) {
				...
				return 
						furnaceBase.getIngredient().getType() == furnaceRecipe.getInput().getType() &&
			...
			if (baseRecipe instanceof CraftRecipe) {
				...
				try {
			        ...
			        for (RecipeItemStack list : items) {
			        	org.bukkit.inventory.ItemStack baseItem = baseItems[ (i / craftBase.getWidth()) * 3 + (i % craftBase.getWidth()) ];
			        	if (baseItem == null) {
			            ...
			            if (list != null && list.choices.length > 0) {
			            	...
			            	for (ItemStack stack : list.choices) {
			            	    org.bukkit.inventory.ItemStack bukkitItem = new org.bukkit.inventory.ItemStack(org.bukkit.craftbukkit.v1_12_R1.util.CraftMagicNumbers.getMaterial(stack.getItem()), 1, (short) stack.getData());
