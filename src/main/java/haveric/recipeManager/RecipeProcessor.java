...
public class RecipeProcessor implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if (!dir.exists() && !dir.mkdirs()) {
                Messages.sendAndLog(sender, RMCChatColor.RED + "Error: couldn't create directories: " + dir.getPath());
            }
            ...
            if (fileList.isEmpty()) {
                ...
                if (errors > 0) {
                    ...
                    Messages.sendAndLog(sender, RMCChatColor.YELLOW + parsed + " " + loaded + " recipes from " + fileList.size() + " files in " + (System.currentTimeMillis() - start) / 1000.0 + " seconds, " + errors + senderMessage);
    ...
    private boolean recipeExists(BaseRecipe recipe) {
        ...
        RMCRecipeInfo registered = getRecipeFromMap(recipe, RecipeManager.getRecipes().index);
        ...
        RMCRecipeInfo queued = getRecipeFromMap(recipe, registrator.queuedRecipes);
